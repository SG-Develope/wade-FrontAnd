        ...
        <div className="bg-white border-b border-pebble px-4 py-2.5 flex gap-1.5 shrink-0">
          {TYPE_FILTER.map(f => (
            <button key={f.key} onClick={() => setFilter(f.key)}
              className={`flex items-center gap-1 text-[11px] px-3 py-1 rounded-full border border-pebble cursor-pointer font-medium transition-all duration-150 ${filter === f.key ? 'bg-river text-white border-river' : 'bg-white text-moss'}`}>
          ...
          {placesLoading ? (
            ...
            <div className="flex flex-col gap-2">
              {filtered.map(place => {
                ...
                return (
                  <div key={place.id} onClick={() => setSelected(isSelected ? null : place.id)}
                    className="bg-white rounded-xl px-3.5 py-3 flex items-center gap-3 cursor-pointer transition-all duration-150"
                    style={{ border: `0.5px solid ${isSelected ? colors.marker : '#EDE8E0'}`, boxShadow: isSelected ? `0 3px 12px ${colors.marker}30` : 'none' }}>
                ...
                <>
                  <div className={`rounded-[10px] px-3 py-2.5 mb-2.5 border ${st.bgCls} ${st.textCls}`} style={{ borderColor: colors.marker }}>
                    ...
                    <div className="text-[10px] mt-0.5 opacity-80">
                      {selectedPlace.stationId === 'yangpo' ? '양포교' : '호국의다리'} 수위 {level != null ? level.toFixed(1) + 'm' : '-'} 기준
                    </div>
                  </div>
                  ...
                  <div className="mb-2.5">
                    ...
                    {[
                      { label: '안전', range: `${selectedPlace.safeWl}m 이하`,                                  color: '#1D9E75' },
                      { label: '주의', range: `${selectedPlace.safeWl}m ~ ${selectedPlace.cautionWl}m`,         color: '#EF9F27' },
                      { label: '위험', range: `${selectedPlace.cautionWl}m 초과`,                               color: '#E24B4A' },
                    ].map(item => (
          ...
          <div>
            <div className="px-4 py-3.5 border-b border-[#F5F0EA]">
              ...
              {places.map(place => {
                ...
                const st     = STATUS_TEXT[status]
                return (
                  <div key={place.id} onClick={() => setSelected(place.id)}

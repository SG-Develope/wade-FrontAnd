...
import { PLACES, getPlaceStatus, STATUS_COLORS, STATIONS } from '@/constants/stations'
  ...
  const renderMarkers = () => {
    ...
    PLACES.forEach(place => {
      ...
      setTimeout(() => {
        ...
        if (typeof el !== 'string') el.addEventListener('click', () => setSelected(place.id))
      }, 100)
          ...
          <div ref={mapRef} className="w-full h-full bg-[#D6EBC8]">
            {!window.kakao?.maps && (
              <div className="w-full h-full flex flex-col items-center justify-center gap-2 bg-gradient-to-br from-[#D6EBC8] to-[#C4DFB0]">
                ...
                <div className="text-[11px] text-moss">index.html의 카카오 JS 키를 교체해주세요</div>
              </div>
            )}
          </div>
        </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] text-moss mt-0.5">
                      연결 관측소: {place.stationId === 'yangpo' ? '양포교' : '호국의다리'} · 수위 {level.toFixed(1)}m
                    </div>
                  ...
                  <div className={`text-[11px] font-bold px-2.5 py-1 rounded-full shrink-0 ${st.bgCls} ${st.textCls}`}>
                    {st.label}
                  </div>
                </div>
              )
        ...
        {selectedPlace ? (
          <div className="p-4">
            ...
            {(() => {
              ...
              const status = getPlaceStatus(selectedPlace, level) as WaterStatus
              const colors = STATUS_COLORS[status]
              const st = STATUS_TEXT[status]
              return (
                <>
                  ...
                  >
                    ...
                    <div className="text-[10px] mt-0.5 opacity-80">
                      {selectedPlace.stationId === 'yangpo' ? '양포교' : '호국의다리'} 수위 {level.toFixed(1)}m 기준
                    </div>
                  </div>
                  ...
                  <div className="mb-2.5">
                    ...
                    {[
                      { label: '안전', range: `${selectedPlace.thresholds.safe}m 이하`, color: '#1D9E75' },
                      { label: '주의', range: `${selectedPlace.thresholds.safe}m ~ ${selectedPlace.thresholds.caution}m`, color: '#EF9F27' },
                      { label: '위험', range: `${selectedPlace.thresholds.caution}m 초과`, color: '#E24B4A' },
                    ].map(item => (
          ...
          <div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-[10px] text-moss">{level.toFixed(1)}m</div>
                    </div>
                    <span className={`text-[9px] px-1.5 py-0.5 rounded-lg font-bold ${st.bgCls} ${st.textCls}`}>
                      {st.label}
                    </span>
                  </div>
                )
            ...
            <div className="px-4 py-3.5">
              ...
              <div className="text-[11px] text-[#4A5A3A] leading-[1.7]">
                장소를 클릭하면 상세 안전 기준과 편의시설 정보를 확인할 수 있습니다.
                수위는 10분 주기로 자동 갱신됩니다.
              </div>
            </div>

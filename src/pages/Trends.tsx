...
export default function Trends() {
  ...
  const { data: historyData, isLoading: historyLoading } = useWaterLevelHistory(
    selectedStation,
    hours,
  );
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-5 bg-white">
          ...
          {STATIONS_META.map((s) => (
            ...
            >
              <span
                ...
                style={{
                  background: selectedStation === s.id ? "#fff" : s.color,
                }}
              />
              {s.label}
            </button>
            ...
            {HOURS_OPTIONS.map((h) => (
              <button
                ...
                className={`text-[11px] px-2.5 py-1 rounded-full border border-pebble cursor-pointer transition-colors ${
                  hours === h
                    ? "bg-sand text-soil font-semibold"
                    : "bg-white text-moss"
                }`}
              >
                {h}h
        ...
        <div className="flex gap-3 mb-3">
          {/* 수위 변화 그래프 (65%) */}
          <div
            className="bg-white border border-pebble rounded-[14px] px-4 py-3.5"
          ...
          >
            <div className="flex items-center justify-between text-[10px] text-moss font-bold tracking-[0.04em] mb-2.5">
              수위 변화 그래프
              <span className="text-[9px] bg-river-light text-river px-1.5 py-0.5 rounded-full font-semibold">
                {hours}시간
              </span>
            </div>
            {historyLoading ? (
        ...
        <div className="border border-pebble rounded-[14px] px-4 py-3.5 min-h-82">
          <div className="text-[10px] text-moss font-bold tracking-[0.04em] mb-2.5">
            최근 관측 이력
          </div>
            ...
            <thead>
              <tr>
                {["시각", "수위", "상태", "평상 대비"].map((h) => (
                  <th
                    key={h}
                    className="text-[10px] text-moss font-bold px-2 py-1.5 border-b border-pebble text-left tracking-[0.03em]"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {history
                ...
                .map((point, i) => {
                  ...
                  const diffColor =
                    diff > 0.5 ? "#E24B4A" : diff > 0 ? "#EF9F27" : "#1D9E75";
                  return (
                    <tr key={i}>
                      <td className="px-2 py-1.5 border-b border-[#F5F0EA] text-moss">
                        {new Date(point.measuredAt).toLocaleTimeString(
                          "ko-KR",
                          { hour: "2-digit", minute: "2-digit" },
                        )}
                      </td>
                      <td className="px-2 py-1.5 border-b border-[#F5F0EA] font-semibold">
                      ...
                      <td
                        className="px-2 py-1.5 border-b border-[#F5F0EA] font-semibold"
      ...
      <div className="w-[308px] bg-white border-l border-pebble overflow-y-auto flex flex-col shrink-0">
        ...
        <div className="px-4 py-3.5 border-b border-[#F5F0EA]">
          <div className="text-[10px] text-moss font-bold tracking-[0.05em] mb-3">
            수위 직관 시각화
          </div>
          {waterLoading ? (
            ...
            <div className="flex justify-around py-2">
              {stations.map((s) => (
                <TubeVisualization key={s.id} station={s} />
              ))}
            </div>
          )}
        </div>
            ...
            return (
              <div key={s.id} className="mb-2.5">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[11px] font-semibold text-soil">
                    {s.name}
                  </span>
        ...
        <div className="px-4 py-3.5">
          ...
          {stations.map((s) => (
            <div key={s.id} className="mb-3">
              <div className="text-[11px] font-semibold text-soil mb-1.5">
                {s.name}
              </div>
              {[
                ...
                <div
                  key={item.label}
                  className="flex items-center justify-between py-1 border-b border-[#F5F0EA] text-[11px]"
                >
